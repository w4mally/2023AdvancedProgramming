...
int read_from_csv(FILE *infp, zipnode_t **table);
...
int main(void){
    ...
    while(1){
        ...
        result = search_node(table[hash(input_addr)], input_addr);
...
int read_from_csv(FILE *infp, zipnode_t **table){
	...
	while (fgets(instr, BUF_SIZE, infp) != NULL) {
        ...
        hashed = hash(fulladdr);

        add_node(&table[hashed], zip, fulladdr);
